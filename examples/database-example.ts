...
import { ValidateSqlQueryResponse, ExtractSqlParametersResponse } from '../src/types/common.js';
...
async function databaseExample() {
  try {
    ...
    if (!connectionString) {
      console.error('Error: NF_TEST_DB_CONNECTION_STRING environment variable is not set');
      console.error('Please set it to a valid PostgreSQL connection string:');
      console.error('Example: export NF_TEST_DB_CONNECTION_STRING=\'postgres://user:password@localhost:5432/dbname\'');
      return;
    }
    ...
    try {
      ...
      console.log('About to call testDatabaseConnection with:', connectionString.replace(/:[^:]*@/, ':****@'));
      const testConnectionResponse = await datasourcesClient.testDatabaseConnection(connectionString);

      if (testConnectionResponse.error) {
        console.error('Error testing connection:');
        if (testConnectionResponse.error.details && testConnectionResponse.error.details.detail) {
          console.error('Details:', JSON.stringify(testConnectionResponse.error.details.detail, null, 2));
        }
        console.error('Please check that your database connection string is correct and the database is accessible.');
      } else {
        console.log('Connection test result:');
        // Type assertion for the response data
        const testData = testConnectionResponse.data as { success: boolean; tables: Array<any> };
        console.log(`- Success: ${testData.success}`);
        console.log(
          `- Tables found: ${testData.tables?.length || 0}`,
        );
        if (
          testData.tables &&
          testData.tables.length > 0
        ) {
            ...
            .forEach((table: any, index: number) => {
              console.log(
                `  ${index + 1}. ${table.name} - Rows: ~${
                  table.estimatedRows
                }, Size: ${table.estimatedSize}, Columns: ${
                  table.columnCount
                }`,
              );
            });
      ...
      console.error('Make sure your database is running and accessible from your current network.');
    }
    ...
    try {
      ...
      if (sampleTablesResponse.error) {
        ...
        if (sampleTablesResponse.error.details && sampleTablesResponse.error.details.detail) {
          console.error('Details:', JSON.stringify(sampleTablesResponse.error.details.detail, null, 2));
        }
        console.error('Please check that your database connection string is correct and the specified tables exist.');
      } else {
        console.log('Sample tables result:');
        // Type assertion for the response data
        const sampleData = sampleTablesResponse.data as { dataObjects: Record<string, string>; jobs: Array<any> };
        console.log(
          `- Data Objects: ${
            Object.keys(sampleData.dataObjects || {}).length
          }`,
        );
        console.log(
          `- Jobs created: ${sampleData.jobs?.length || 0}`,
        );

        if (
          sampleData.jobs &&
          sampleData.jobs.length > 0
        ) {
          console.log('Jobs:');
      ...
      console.error('Make sure your database is running and the specified tables exist.');
    }
    ...
    try {
      ...
      if (executeCustomSqlResponse.error) {
        ...
        if (executeCustomSqlResponse.error.details && executeCustomSqlResponse.error.details.detail) {
          console.error('Details:', JSON.stringify(executeCustomSqlResponse.error.details.detail, null, 2));
        }
        console.error('Please check that your SQL query is valid and the database is accessible.');
      } else {
      ...
      console.error('Make sure your SQL query is valid for your database schema.');
    }
    ...
    try {
      // Use the datasourcesClient for database operations
      // Note: The client method will convert camelCase to snake_case internally
      const validateSqlSyntaxResponse = await datasourcesClient.validateSqlSyntax({
        connectionString,  // Will be converted to connection_string
        sqlQuery,         // Will be converted to sql_query
      });
      ...
      if (validateSqlSyntaxResponse.error) {
        ...
        console.error('Please check that your SQL query syntax is correct for your database type.');
      } else {
        console.log('SQL syntax validation result:');
        // Type assertion for the response data
        const syntaxData = validateSqlSyntaxResponse.data as { valid: boolean; message?: string };
      ...
      console.error('Make sure your SQL query syntax is correct for your database type.');
    }
    ...
    try {
      ...
      if (validateSqlQueryResponse.error) {
        ...
        console.error('Please check that your SQL query is valid and the max_rows parameter is appropriate.');
      } else {
        console.log('SQL query validation result:');
        // Use the proper type from the imported interfaces
        const queryData = validateSqlQueryResponse.data as ValidateSqlQueryResponse;
      ...
      console.error('Make sure your SQL query is valid and returns fewer rows than max_rows.');
    }
    ...
    try {
      ...
      // Use the datasourcesClient for database operations
      const extractSqlParametersResponse = await datasourcesClient.extractSqlParameters(
      ...
      if (extractSqlParametersResponse.error) {
        ...
        console.error('Please check that your SQL query contains properly formatted parameters like {{parameter_name}}.');
      } else {
        console.log('SQL parameters extraction result:');
        // Use the proper type from the imported interfaces
        const paramsData = extractSqlParametersResponse.data as ExtractSqlParametersResponse;
        console.log(
          `- Parameters found: ${
        ...
        console.log(
          `- Parsed Query: ${paramsData.parsedQuery}`,
        );
      ...
      console.error('Make sure your SQL query contains properly formatted parameters like {{parameter_name}}.');
    }
