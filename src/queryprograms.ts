...
import { get, post, put, del, postStream, patch } from './client';
...
export const queryProgramsApi = {
  listQueryPrograms: async (
    ...
    return get<QueryProgram[]>(`/v1/queryprograms`, { params: params });
  },

  getQueryProgram: async (id: string): Promise<ApiResponse<QueryProgram>> => {
    return get<QueryProgram>(`/v1/queryprograms/${id}`);
  },

  getQueryProgramsByProject: async (
    project_id?: string
  ): Promise<ApiResponse<QueryProgram[]>> => {
    return get<QueryProgram[]>(`/v1/queryprograms`, {
      params: { project_id }
    });
  },
  ...
  createQueryProgram: async (
    ...
    return post<QueryProgram>('/v1/queryprograms', { body: filteredParams });
  },
  ...
  updateQueryProgram: async (
    ...
    return patch<QueryProgram>(`/v1/queryprograms/${id}`, {
      body: filteredParams
    });
  },

  deleteQueryProgram: async (id: string): Promise<ApiResponse<void>> => {
    return del<void>(`/v1/queryprograms/${id}`);
  },
  ...
  executeQueryProgram: async (
    ...
    if (params?.stream) {
      ...
      return post<QueryResponse>(`/v1/queryprograms/${id}/execute`, {
        params: params?.input_data || {}
      });
    }
  },
  ...
  getQueryProgramHistory: async (
    ...
    return get<QueryResponse[]>(`/v1/queryprograms/${id}/history`, {
      params: params
    });
  },
  ...
  publishQueryProgram: async (
    ...
    return patch<QueryProgram>(`/v1/queryprograms/${id}/publish`);
  }
};
