import { get, getStream, post, postStream } from './client';
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('id:')) {
        ...
        try {
          ...
          return {
            error: `Invalid JSON in event data: ${e instanceof Error ? e.message : e}`
          };
        }
      }
    }
    ...
    if (eventObj.event_type === 'NewDataObjectEvent') {
      if (eventObj.data?.data_lineage) {
        try {
          eventObj.data.data_lineage = JSON.parse(eventObj.data.data_lineage);
        } catch (e) {
          return {
            error: `Invalid JSON in data_lineage: ${e instanceof Error ? e.message : e}`
          };
        }
      }
      if (eventObj.data?.data_object) {
        try {
          eventObj.data.data_object = JSON.parse(eventObj.data.data_object);
        } catch (e) {
          return {
            error: `Invalid JSON in data_object: ${e instanceof Error ? e.message : e}`
          };
        }
      }
    }
...
export const jobsApi = {
  ...
  submitJob: async (params: SubmitJobParams): Promise<ApiResponse<string>> => {
    return post<string>('/v1/jobs/submit', {
      body: params
    });
  },
  ...
  getJobHistory: async (jobId: string): Promise<ApiResponse<any>> => {
    return get<any>(`/v1/jobs/history/${jobId}`);
  },
  ...
  listActiveWorkers: async (): Promise<ApiResponse<Worker[]>> => {
    return get<Worker[]>('/v1/jobs/workers/active');
  },
  ...
  getJobsBySource: async (params: {
    ...
    return get<any>('/v1/jobs/by-source', {
      params: {
